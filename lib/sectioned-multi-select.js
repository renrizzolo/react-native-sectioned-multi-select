...
class SectionedMultiSelect extends PureComponent {
  ...
  _filterItems = (searchTerm) => {
    ...
    if (isFilterSotero) {
      let searchText = searchTerm.replace(/[\^$\\.*+?()[\]{}|]/g, '').trim().toLowerCase()

      items && items.forEach((item) => {
        if(item.label.toLowerCase().match(searchText)) {
  ...
  AlterColorButtonPrimary = (items) => {
    const { colors } = this.state;

    if (typeof items === 'undefined') return;

    if (!items.length) {
      colors.primary = '#17668E';
  
      if (!isEqual(this.state.colors, colors)) {
        this.setState({ colors: StyleSheet.flatten([defaultColors, colors]) });
      }

      return items;
    }

    colors.primary = '#7C7C7C';
          ...
          >
            ...
            {!hideSearch && (
              <View style={[{ flexDirection: 'row', paddingVertical: 5 }, styles.searchBar]}>
                ...
                <TouchableOpacity
                  style={{ alignItems: 'center', justifyContent: 'center', marginRight: 10, paddingRight: 7, paddingLeft: 7 }}
                    ...
                    {!renderItems || (!renderItems.length && !searchTerm)
                      ...
                      <View>
                        <FlatList
                          ...
                          data={this.AlterColorButtonPrimary(renderItems)}
                          extraData={selectedItems}
                          keyExtractor={item => `${item[uniqueKey]}`}
                          ItemSeparatorComponent={this._renderSeparator}
                          ListFooterComponent={this._renderFooter}
                          renderItem={this._renderItemFlatList}
                          ListEmptyComponent={() => callIfFunction(noResultsComponent)}
                          {...itemsFlatListProps}
                        />
                      </View>
                  </View>
              ...
              >
                {!hideConfirm && (
                  <Touchable
                    ...
                    onPress={() => this._submitSelection(true)}
                    style={{
                      width: 'auto',
                      justifyContent: 'center',
                      alignItems: 'center',
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[{ fontSize: 14, color: '#ffffff' }, confirmFont, styles.confirmText]}
                      >
                        {confirmText}
                      </Text>
                    </View>
