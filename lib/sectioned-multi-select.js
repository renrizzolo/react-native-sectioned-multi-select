...
class SectionedMultiSelect extends PureComponent {
	...
	_highlightChildren = (id) => {
		...
		console.log('hi', highlighted, this.state.highlightedChildren);
	...
	_unHighlightChildren = (id) => {
		...
		this.setState({highlightedChildren: highlighted})
		console.log('unhi', id, this.state.highlightedChildren)

	}

	_selectChildren = (id) => {
		const { items, selectedItems, uniqueKey, subKey, highlightChildren } = this.props
		let i = 0
		const selected = []
		for (; i < items.length; i++) {
			if (items[i][uniqueKey] === id && Array.isArray(items[i][subKey])) {
