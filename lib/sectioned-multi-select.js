...
class SectionedMultiSelect extends PureComponent {
  ...
  _filterItems = (searchTerm) => {
    ...
    let newFilteredItems = []
	  ...
	  items.forEach((item) => {
		  const parts = removerAcentos(searchTerm.trim()).split(/[[ \][)(\\/?\-:]+/)
		  const regex = new RegExp(`(${parts.join('|')})`, 'i')


		  const newKey = `_${displayKey}`;
		  item.children.forEach((value, index) => {
			  value = {...value, [newKey]: removerAcentos(value[displayKey])}
			  item.children[index] = value;
		  });

		  if (regex.test(this.getProp(item, newKey))) {
		  ...
		  if (item[subKey]) {
			  ...
			  item[subKey].forEach((sub) => {
				  if (regex.test(this.getProp(sub, newKey))) {
        ...
        >
          <View style={[{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)' }, styles.backdrop]}>
            ...
            >
              ...
              <ScrollView style={[{ paddingHorizontal: 12, flex: 1 }, styles.scrollView]}>
                { (items && items.length) ?
                  <View>
                    {renderItems.length ?
                      <View>
