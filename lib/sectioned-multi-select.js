...
class SectionedMultiSelect extends PureComponent {
  ...
  _filterItems = async (searchTerm) => {
    ...
    if (filterItems) {
      const fn = filterItems(searchTerm, items, this.props)
      const data = await fn
      console.log('returning', data)
      console.log('items', items);

      this.setState({ filterResults: data })
      return
    }
    let filteredItems = []
    ...
    this.setState({ filterResults: filteredItems })
  ...
  render() {
    ...
    const renderItems = searchTerm ? this.state.filterResults : items
