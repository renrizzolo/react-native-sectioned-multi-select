...
class SectionedMultiSelect extends PureComponent {
  ...
  static propTypes = {
    ...
    toggleIconComponent: PropTypes.object,
  ...
  _filterItems = (searchTerm) => {
    const {
      items, subKey, uniqueKey, showDropDowns,
    } = this.props;
    let filteredItems = [];
  ...
  _toggleItem = (item, parent) => {
    ...
    if (single) {
      ...
      if (selected) {
        ...
        if (parent) {
          if (selectChildren) {
            console.log('selectch unselected');

            newItems = [...newItems, ...this._selectChildren(item[uniqueKey])];
          } else if (highlightChildren) {
            console.log('highlihgtc unselected');
  ...
  _unHighlightChildren = (id) => {
    ...
    array['0'][subKey].reduce((prev, curr) => {
      highlighted.splice(highlighted.findIndex(el => (
          el === curr[uniqueKey]
        )), 1);
    }, {});
  ...
  _selectChildren = (id) => {
    const {
      items, selectedItems, uniqueKey, subKey, highlightChildren,
    } = this.props;
    let i = 0;
    const selected = [];
    for (; i < items.length; i++) {
  ...
  _rejectChildren = (id) => {
    ...
    const array = items.filter(item => item[uniqueKey] === id)['0'][subKey];

    array.reduce((prev, curr) => {
      selectedItems.splice(selectedItems.findIndex(el => (
          el === curr[uniqueKey]
        )), 1);
    }, {});
  ...
  >
    <TouchableOpacity
      ...
      style={[{
            ...
            paddingVertical: 6,
          }, this.styles.item, this._itemSelected(item) && this.styles.selectedItem]}
    >
      <Text
  ...
  style={{
    ...
    >
      { this._showSubCategoryDropDown(item[uniqueKey]) ?
              ...
              dropDownToggleIconUpComponent || <Icon
  ...
  style={{
              ...
              />
              :
              dropDownToggleIconDownComponent || <Icon
  ...
  _rowSubItem = (item) => {
    ...
    const highlightChild = highlightedChildren.includes(item[uniqueKey]);
  ...
  >
    <TouchableOpacity
      ...
      style={[{
            ...
            justifyContent: 'flex-start',
            paddingVertical: 6,
          }, this.styles.subItem, (this._itemSelected(item) || highlightChild) && this.styles.selectedItem]}
    >
      <Text
