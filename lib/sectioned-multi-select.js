...
class SectionedMultiSelect extends PureComponent {
	...
	_renderItem = (item, parent) => {
		const { subKey, uniqueKey } = this.props
		return(
			<View>
				{this._rowItem(item, parent)}
			</View>
		)
	}
			...
			<View>
				...
				<View>
					...
					<FlatList
						...
						renderItem={rowData => this._rowItem(rowData.item, false)}
						/>
					}
			{/*	{item[subKey] && item[subKey].map((subItem, i) => (
									this._rowItem(subItem, false)
								))}*/}
				</View>
				}
			</View>
	...
	_rowItem = (item, parent) => {
		...
		const hasDropDown = parent && item[subKey] && item[subKey].length > 0 && showDropDowns
		...
		return (
			<View key={item[uniqueKey]} style={{flexDirection:'row', flex:1,backgroundColor: parent ? colors.itemBackground : colors.subItemBackground}}>
				<TouchableOpacity
					...
					{hasDropDown &&
						<TouchableOpacity style={[{alignItems:'flex-end', justifyContent:'center', paddingLeft:10,}, this.styles.toggleIcon]} onPress={() => {this._toggleDropDown(item[uniqueKey])}}>
	...
	componentWillMount() {
		// const { items, subKey } = this.props;
		// const data = []
		// items.forEach((item, i) => {
		// 	data.push({data: item[subKey] || [], item})
		// 	console.log(item)
		// 	}		
		// )
	...
	render() {
		...
		// format items for SectionList and allow for dropdown toggles to work. It's way too slow rendering this all the time.
		// const data = []
		// renderItems.forEach((item, i) => 
		// 	data.push({data: this._showSubCategoryDropDown(item[uniqueKey]) ? item[subKey] : [], item})							
		// )
				...
				<View style={[{flex:1, backgroundColor:'rgba(0,0,0,0.5)'}, this.styles.backdrop]}>
					...
					<View style={{flexDirection: 'row', paddingVertical:5}, [this.styles.searchBar]}>
						...
						<TextInput
							...
							style={[{ flex: 1, fontSize:18, paddingVertical: 8 },	searchTextFont, this.styles.searchTextInput]}
						/>
					</View>
					<View style={[{ paddingHorizontal:15, flex:1, }, this.styles.scrollView]}>
					{ renderItems.length ?
						<View>
				 ...
				 <FlatList
						...
						{/*	<SectionList
									...
									renderSectionHeader={({section}) => this._renderItem(section, true)}
					...
					<Touchable
						...
						<View style={[{	
							...
							<Text style={[{fontSize: 18, color:'#ffffff'}, confirmFont, this.styles.confirmText]}>{confirmText}</Text>
						</View>
					</Touchable>
					</View>
				</View>
