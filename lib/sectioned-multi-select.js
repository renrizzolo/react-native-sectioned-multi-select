...
class SectionedMultiSelect extends Component {
	constructor(props) {
		super(props)
	...
	_rowItem = (item, parent) => {
		...
		const hasDropDown = parent && item[subKey] && showDropDowns
		...
		return (
			<View key={item[uniqueKey]} style={{flexDirection:'row', flex:1}}>
				<TouchableOpacity
					...
					{hasDropDown &&
						<TouchableOpacity style={[{alignItems:'center', justifyContent:'flex-end', marginLeft:10,}, this.styles.toggleIcon]} onPress={() => {this._toggleDropDown(item[uniqueKey])}}>
							
							<Icon
								name={this.state.showSubCategories.includes(item[uniqueKey]) ? "keyboard-arrow-up" : "keyboard-arrow-down"}
								size={22}
								style={{
